...
segprefix = 'K_' # Kidney segmentation extension
cystprefix = 'CY_' # Edge-Core segmentation extension
...
for filename in tqdm(files):
	...
	if segprefix in filename:
		#try:
			...
			#pre-process
			data=np.asarray(data).astype(np.float32)#make data into array of float32
			...
			#preallocate zeros matrix the size of scan by scan and 4 deep			
			...
			for io in range(0,np.shape(segdata)[2]): 
				...
				if io==0:
					...
					dataslice1 = cv2.resize(data[:,:,io-1], (Scan,Scan), interpolation=cv2.INTER_CUBIC)#resize using interpolation
